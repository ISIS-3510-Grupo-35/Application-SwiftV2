...
class ParkingLotViewModel: ObservableObject {
    ...
    private var parkingLotsCopy = [ParkingLot]()
    ...
    func updateParkingLotsForLocation() {
        let filteredParkingLots = parkingLots.filter({
            $0.address.lowercased() == searchLocation.lowercased() ||
            $0.name.lowercased() == searchLocation.lowercased()
        })
        
        self.parkingLots = filteredParkingLots.isEmpty ? parkingLotsCopy: filteredParkingLots
    }
